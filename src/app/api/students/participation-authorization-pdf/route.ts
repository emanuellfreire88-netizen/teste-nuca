...
/**
 * POST /api/students/participation-authorization-pdf
 *
 * Generates an "AUTORIZAÇÃO DE PARTICIPAÇÃO" PDF for one or more students.
...
export const POST = withRole(['Admin', 'Operator'], async (req: AuthenticatedRequest) => {
  try {
    ...
    for (const student of students) {
      ...
      // ── Title ──
      drawText(page, 'AUTORIZAÇÃO DE PARTICIPAÇÃO', MARGIN_LEFT, y, {
        font: fontBold,
        size: 16,
        color: BLUE_DARK,
      ...
      drawBlankField(page, 'Telefone:', student.guardian_phone, MARGIN_LEFT + 8 + halfW, y, halfW, 52);
      y -= 28;
      ...
      const para1 = `Eu, identificado(a) acima como responsável legal pelo(a) adolescente ${student.full_name}, autorizo sua participação nas atividades promovidas pelo NUCA – Núcleo de Cidadania de Adolescentes, reconhecendo a importância das ações voltadas ao desenvolvimento da cidadania, participação social, formação pessoal e fortalecimento dos direitos de crianças e adolescentes.`;

      y = drawWrappedText(page, para1, termX, y, termMaxW, { size: 9, lineHeight: 13 });
      y -= 6;

      // Paragraph 2: Awareness of activity types
      const para2 = 'Declaro estar ciente de que as atividades poderão compreender encontros, oficinas, palestras, capacitações, campanhas, eventos, visitas técnicas, ações comunitárias, atividades culturais, esportivas e educativas, realizadas nas dependências do município ou em outros locais previamente informados pela coordenação do NUCA.';

      y = drawWrappedText(page, para2, termX, y, termMaxW, { size: 9, lineHeight: 13 });
      y -= 6;

      // Paragraph 3: Commitment to communicate
      const para3 = 'Comprometo-me a comunicar à coordenação qualquer informação relevante que possa interferir na participação do(a) adolescente, bem como manter meus dados de contato atualizados.';

      y = drawWrappedText(page, para3, termX, y, termMaxW, { size: 9, lineHeight: 13 });
      y -= 6;

      // Paragraph 4: Awareness of norms and authorization of participation
      const para4 = 'Declaro ainda que estou ciente das normas de participação do programa e autorizo o(a) adolescente acima identificado(a) a participar das atividades desenvolvidas pelo NUCA durante o período em que permanecer regularmente inscrito(a) no programa.';

      y = drawWrappedText(page, para4, termX, y, termMaxW, { size: 9, lineHeight: 13 });
      y -= 18;

      // ════════════════════════════════════════════════════════════════════════
      // SECTION 4: DECLARAÇÃO
      // ════════════════════════════════════════════════════════════════════════
      y = drawSection(page, 'DECLARAÇÃO', MARGIN_LEFT, y, CONTENT_WIDTH, BLUE_DARK);

      const declText = 'Declaro que li, compreendi e concordo com os termos desta autorização, assumindo inteira responsabilidade pelas informações prestadas.';
      y = drawWrappedText(page, declText, termX, y, termMaxW, { font: fontBold, size: 9, lineHeight: 13 });
      y -= 20;

      // ════════════════════════════════════════════════════════════════════════
      // SECTION 5: ASSINATURA
      // ════════════════════════════════════════════════════════════════════════
    ...
    await logAction(
      req.user!.userId,
      'export_report',
      `Geração de Autorização de Participação (PDF): alunos=${students.length}`,
      req
    );
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="autorizacao-participacao.pdf"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Participation Authorization PDF error:', error);
    return NextResponse.json(
      { error: 'Erro ao gerar autorização de participação' },
      { status: 500 }
    );
  }
});
